...
The evolution of Common Criteria (CC) evaluations into cloud environments reflects the growing need to address security in modern IT systems. To effectively support this transition, Protection Profile (PP) authors should focus on aligning their assumptions and objectives for the operational environment with established cloud authorization frameworks such as FedRAMP, C5, and SecNumCloud. These frameworks offer a wealth of operational controls and assurances that are critical for defining trusted platforms and addressing jurisdictional challenges, such as data sovereignty requirements.

This alignment is not merely a convenience—it is foundational to the success of cloud-based CC evaluations. Cloud authorization frameworks already validate key aspects of a cloud service provider’s (CSP) operational environment, such as access controls, physical security, and regulatory compliance. By leveraging these authorization programs, PP authors can build a bridge between the operational realities of cloud deployments and the formalized assurance processes required by CC. This integration allows CC schemes to refine their criteria for trusted platforms in a way that is both practical and globally relevant.

For example, cloud frameworks inherently address concerns such as multi-tenancy, shared resource access, and data residency through their authorization processes. When PP authors align their assumptions and objectives with these frameworks, they ensure that CC evaluations consider not only the technical functionality of a Target of Evaluation (TOE) but also the broader operational and compliance context in which it exists. This alignment provides evaluators with a clear, actionable foundation for assessing the suitability of the operational environment while simultaneously addressing critical issues like sovereignty and regulatory adherence.

As PPs are updated to support cloud evaluations, they must also account for scenarios where the TOE operates on a trusted platform or forms part of the trusted platform’s internal infrastructure. This is particularly relevant for solutions like virtualization platforms, software-defined networking components, and operating systems that are integral to cloud operations. By extending CC evaluations into these areas, PP authors help raise the overall assurance bar for cloud environments.

This creates a bidirectional and symbiotic relationship between CC and cloud authorization frameworks. While cloud frameworks validate operational controls and compliance, they lack the formal functional testing that is the foundation of CC evaluations. CC fills this gap by formally evaluating a TOE’s security claims and functional behavior. Together, these approaches complement one another: CC evaluations benefit from the operational assurances provided by cloud frameworks, and cloud frameworks gain credibility through the assurance CC brings to individual components.

Ultimately, this alignment ensures that as cloud adoption continues to grow, both approaches evolve in tandem to meet the increasing complexity and security demands of modern IT systems. By addressing cloud-specific challenges in a structured, collaborative manner, Protection Profiles can facilitate evaluations that are both technically robust and operationally grounded, driving greater trust in cloud-based solutions.
...
Additionally, the Security Target (ST) ASE requirements can be modified by PP authors to provide clear explanations by PP authors to provide clear explanations to ensure evaluators have sufficient information and understanding regarding cloud infrastructure and security. Such modifications may be appropriate for the TOE Summary Specification (TSS) requirements and should include detailed information such that it is clear how the TSFI or Security Functional Requirement (SFR) enforcing features interact with the TOE platform in a cloud context. 
...
Not all evaluations of a TOE will rely upon a TOE platform. If the PP does not prescribe a TOE platform a Trusted Platform is still needed to provide a hosting environment. 
...
Schemes should review the PPs guidance around vulnerability testing as well and confirm that the evaluator has considered that guidance. The evaluator evidence should demonstrate that the evaluator was able to use cloud attack vectors. (e.g. service portal, management plane, etc. to complete the testing).

Additionally, guidance for penetration testing may be given with the understanding that the TOE may not be deployed in a fully closed environment. As such, there may be limitations on the rules of engagement that must be followed with the CSP. It is expected that evaluation schemes will provide guidance on the penetration test efforts that will be accepted for cloud evaluations, and PP authors should incorporate such guidance into PPs for consistency.

== Guidance for Evaluators

A PP should provide an evaluator with clear explanations tailored to ensure an evaluator has sufficient information and understanding regarding cloud infrastructure and security. Evaluators should aim to develop their knowledge and techniques for TOEs in the cloud to a comparable level to that for traditional on-prem TOEs.

Evaluators should read the PP’s explicit guidance for TOEs in a cloud environment. This should include a mapping, that may be used by an evaluator to confirm that the selected trusted platform has been validated by an appropriate cloud authorization scheme, to have controls fulfilling the assumptions and associated security objectives for the operational environment. This material should be reviewed and considered carefully. In the case where an evaluator considers that the material provided is insufficient or in error, the evaluator should contact the PP author for further discussion.

Evaluators should consider guidance in the protection profile regarding testing. An evaluator should prepare and/or maintain cloud testing infrastructure for the purposes of conducting evaluations. This environment will establish controls provided by the CSP to ensure effective isolation to maintain the integrity of the evaluation results. This may involve isolated Virtual Local Area Networks (VLANs), Access Control Lists (ACL), Compute Resources, etc. on multi-tenant infrastructure. This information will need to be captured and presented to evaluation authorities as evaluation deliverables.
...
Please refer to this link:https://aws.amazon.com/getting-started/cloud-essentials/[link] for more information and the most recent updates to the Cloud Platform.
...
Please refer to this link:https://azure.microsoft.com/en-us/get-started[link] for more information and the most recent updates to the Cloud Platform.
...
Operating system version and the corresponding images may be used, or a custom image can be provided.
...
Please refer to this link:https://docs.oracle.com/en/learn/oci-basics-tutorial/[link] for more information and the most recent updates to the Cloud Platform.

== Cloud Equivalence Considerations

When writing Security Targets, the Cloud Service Offering (CSO) must be detailed in the TOE evaluated configuration and shall include the cloud region and/or applicable datacenter. CSOs offered by the same CSP cannot inherently be assumed to be equivalent. For instance, CSPs may have separate environments among government or commercial customers with differing security capabilities. Therefore, an equivalency rationale must be made to claim multiple CSO compatibility with the evaluated configuration. However, if cloud authorizations exist for multiple regions or datacenters in which a CSO is found to meet similar requirements consistently this may be leveraged as equivalency rationale. In addition, each CC scheme has the option to define a policy for equivalency acceptance criteria.
...
TOEs that are being evaluated in a cloud context are expected to be able to conclusively demonstrate knowledge of the underlying TOE platform for these specifics. If SFR enforcing functionality is agnostic to the TOE platform, such equivalency claims may be made. However, if the TOE is relying on the TOE platform for any SFR enforcing functionality, then the TOE must be able to conclusively demonstrate knowledge of the underlying TOE platform for this functionality.
...
While concerns have been raised about entropy differences in cloud deployments, particularly regarding entropy collection, it's important to understand the full context of entropy generation and conditioning in modern systems. The diagram below illustrates how entropy can be collected at different layers, from hardware sources through to userspace applications:

.Userspace Jitter 
image::images/jitter.png[Jitter Entropy Implementations]

When evaluating entropy sources in cloud environments, two key considerations emerge:

The quantity of entropy may vary across different virtual environments and hardware platforms
The quality of the conditioned entropy output remains consistent when proper conditioning is applied

One approach to maximize the entropy needs in Cloud Native operation is to implement jitter-based entropy collection entirely in userspace (as shown in the containerized application above). Variations in underlying hardware characteristics can affect the raw entropy rate. However, this does not necessarily impact the security of cryptographic operations when:

* Proper entropy assessment per SP 800-90B is performed
* Appropriate conditioning methods are applied to the raw entropy
* Conservative estimates of entropy are used in key generation

For Common Criteria evaluations, this means that entropy assessment should focus on:

The conditioning methods applied to raw entropy
The health testing of entropy sources
The conservative estimation of entropy rates
The proper integration of multiple entropy sources where available

PP Authors must use these considerations and adapt their SFRs and selections appropriately. This approach combined with the jitter entropy implementation above will enable widespread equivalency and hardware agnosticism across offerings within a Cloud Service Provider.

== Protection Profile Considerations

The following section provides a high-level overview for adapting CCitC methodology to existing PPs, including examples where appropriate. This approach is predicated on determining the suitability of a PP for cloud evaluations. Considerations were given to the security problem definition, security objectives, how security requirements are affected by cloud evaluations, and how these evaluations may relate to other cloud authorization schemes recognized by national schemes.
...
** Note that these requirements do not replace a certification of the Cloud Service Offering.
...
** It may be the case that a requirement simply does not apply to a cloud environment, in which case the PP author could consider defining that requirement as an implementation-based SFR that applies only to the on-premise use case. For instance, implementation-based SFRs can be seen via Use Cases and "Conditionally Mandatory" SFRs in the cPP_HCD_v1.0e.
...
* Class ATE (Testing) – As discussed previously in <<Cloud Equivalence Considerations>>, functional testing of SFR claims may or may not be different when the TOE is evaluated in the cloud. For cases where on-premise and cloud evaluation of a given SFR may differ, the PP author is expected to provide clear guidance as to the evaluation activities that are different for each use case.
...
* The same notion of needing the operational environment to be a “trusted platform” applies here – deploying the TOE in a cloud platform and infrastructure that has undergone a third-party authorization is important because it helps the evaluator understand the extent to which the TOE relies on the platform and assures that a vulnerable platform does not introduce any significant potential exploits of the TOE itself.
...
The PP writers may wish to consider whether existing threats, assumptions, and organization security policies should be refined to provide more explanation in the case of cloud.  For example, threat models may consider the impact of potential ubiquitous access and multi-tenancy to either add to existing threats, assumptions, and organizational security policies or refine them.
...
It should be noted that the these suggested additions for a TOE hosted on a trusted platform do not necessarily replace all the assumptions and security objectives for the OE. For example, assumptions around restrictions on general-purpose computing capabilities, not providing through traffic protection, trusted admin at the level of the TOE, non-malicious/trusted/proper users, and TOE updates are unlikely to be fulfilled by the trusted platform.
...
Trusted Platform Security Administrators are trusted to follow and act consistently with, the underlying cloud authorization collateral for Trusted Platforms.
...
If there is existing documentation for a CSP it should be leveraged. Consultants and vendors shall provide necessary supplemental guidance as it supports deploying the TOE as evaluated in the cloud. In addition, functionality that is included but not evaluated shall be clearly identified. 
...
Not all TOE developers will procure cloud resources for Common Criteria Evaluations and certain CCTL's may wish to facilitate this hosting for cloud evaluations. As such, an evaluator should be prepared to create and offer cloud testing infrastructure to TOE developers. This environment must establish controls provided by the CSP to establish effective isolation equivalent to and/or representative of the on-prem environment to ensure the integrity of results. This may involve isolated VLANs, ACLs, Compute Resources, etc. on multi-tenant infrastructure. This information will need to be captured and presented to Evaluation Authorities as evaluation deliverables. Best practices for test environments to expand on these controls for various stakeholders may be forthcoming in future CCiTC publications.
...
In general, the AVA methodology shall be sufficient for most TOE types. However, certain edge cases may present themselves. Evaluators must consider the TOE deployment guidance and identify relevant cloud attack vectors. For instance, a very common vulnerability in cloud computing use cases is the improper configuration of block object storage resources to be publicly accessible with no access control.
...
However, when applicable vulnerabilities are discovered or suspected for TOEs operating in a cloud environment, the means by which evaluators are expected to shift negative test coverage must use cloud attack vectors. (e.g. service portal, management plane, etc.)
...
* The evaluation authority determines sufficiency and may publish a policy. See <<Scheme Guidance>>
