...
The {iTC-shortname} will develop evaluation collateral and guidance to extend CC evaluations into cloud scenarios.
...
In order to avoid updating this ToR, and potentially requiring another submission for approval, the key persons are defined in {iTC-longname} Technical Community Key persons and affiliations <<2>> provided separately at the Team lab file area of the iTC.
...
This {iTC-shortname} follows the processes defined in Establishing Technical Communities and Developing Collaborative Protection Profiles <<3>> with the version agreed with the CCDB Liaison. The version should normally be the latest version approved by the CCRA CCDB {iTC-shortname} Working Group, or the CCRA CCDB as applicable. 
...
== Policies
=== Membership
This iTC is open to all interested persons who have an active role in this technology area and in CC evaluation. To join the iTC, each must submit an application located https://docs.google.com/forms/d/1hQ6c1Zsr7rO3OT6B0GvNPrX5IeC7RPZxjC8Z1XcJQC4[here].
...
* Major Decisions shall be made via the iTC collaboration tools in github. The {iTC-shortname} will provide one or two weeks to make the decision. Minor decisions, such as regular commitments to draft iTC documents, may be made by acclimation during scheduled iTC meetings.
...
* [#2]#[2]# {iTC-longname} Technical Community - Key Persons and Affiliations, {iTC-website}[Online] (link when available)
